...
var Join = require('./query/expr/join');
...
QueryBuilder.prototype = {
	...
	getRootAliases: function() {
		...
		this.getTableMap().forEach(function(obj){
			aliases.push(obj.alias);
		});
	...
	getRootDocuments: function() {
		...
		this.getTableMap().forEach(function(obj){
			...
			var documentName = this._em
				.mapper
				.mapCollectionNameToDocumentName(obj.table);
	 ...
	 * @see innerJoin
	 */
	join: function(table, alias, condition) {
		// TODO : finish
		return this;
	} ,

	/**
	 * Join a table based on metadata mapping information
	 *
	 ...
	 * @returns {QueryBuilder}
	 */
	joinDocument: function(table, alias, condition) {
		// TODO : finish
		return this;
	} ,
	 ...
	 * @throws Error
	 */
	joinTable: function(table, alias, condition) {
		...
		if (condition.constructor.name === 'Comparison') {

			parentAlias = findAlias(condition.getLeftExpr());
			if (!parentAlias) {

				parentAlias = findAlias(condition.getRightExpr());
			}
		...
		var rootAlias = this.findJoiningAlias(alias, parentAlias);
		if (!rootAlias) {
		...
		if (!condition) {
			...
			if (typeof tableMap[rootAlias] !== 'undefined') {
		...
		var join = new Join(Join.TYPE_INNER, table, alias, condition);
