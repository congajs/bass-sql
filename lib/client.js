...
Client.prototype = {
	...
	startTransaction: function(cb) {
		this.rawQuery('START TRANSACTION', null, cb);
	} ,
	...
	rollbackTransaction: function(cb) {
		this.rawQuery('ROLLBACK', null, cb);
	} ,
	 ...
	 * @param {Array,<String>} values Array of values to search for
	 * @param {Object|null} sort Object hash of field names to sort by, -1 value means DESC, otherwise ASC
	 * @param {Function} cb Callback function
	 */
	findWhereIn: function(collection, field, values, sort, cb) {

		var sql = 'SELECT `' + collection + '`.* ';
		sql += 'FROM `' + collection + '` ';
		sql += 'WHERE `' + collection + '`.`' + field + '` IN ( ' + ((new Array(ids.length + 1)).join('?,').replace(/,*$/g, '')) + ') ';
		...
		if (typeof sort !== 'undefined' && sort !== null)
		{
			...
			{
				if (sort[i] === -1){
					sorts.push('`' + collection + '`.`' + i + '` DESC ');
				}
				else
				{
					sorts.push('`' + collection + '`.`' + i + '` ASC ');
				}
			}
