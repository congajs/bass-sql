...
Mapper.prototype = {
	...
	convertRelationsToData: function(metadata, model, data, cb) {
		...
		// embed-one
		for (i in metadata.embeds['one']){
			relation = metadata.embeds['one'][i];

			data[relation.field] = this.convertJavascriptToDb('object', model[relation.field]);
		}

		// embed-many
		for (i in metadata.embeds['many']){
			relation = metadata.embeds['many'][i];
